...
processed_dir=${input_dir}/processed_genia_filtered_merged
...
#python ${CDR_IE_ROOT}/src/processing/utils/biocreative_VI_task5.py --input_dir ${input_dir}/chemprot_test --split test --filter_extra_relations True
...
python ${CDR_IE_ROOT}/src/processing/labled_tsv_to_tfrecords.py --text_in_files ${processed_dir}/\*tive_\*CDR\*train.txt --out_dir ${proto_dir} --max_len ${max_len} --num_threads 10 --multiple_mentions --tsv_format --min_count ${min_count}
# only print dev within the same sentence
python ${CDR_IE_ROOT}/src/processing/labled_tsv_to_tfrecords_single_sentences.py --text_in_files ${processed_dir}/\*tive_\*CDR\*dev.txt --out_dir ${proto_dir} --max_len ${max_len} --num_threads 10 --multiple_mentions --tsv_format --min_count ${min_count} --full_abstract True --load_vocab  ${proto_dir}
